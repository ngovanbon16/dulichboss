...
<head>
    ...
    <script type="text/javascript">
        function sua(row)
        {
            ...
            $("#Q_MA").val(dataRecord.Q_MA);
            $("#Q_TEN").val(dataRecord.Q_TEN);
        ...
        function chitiet(id)
        {
            ...
            //setTimeout("location.href = '<?php echo base_url(); ?>index.php/quyen/detail/"+id+"';",0);
        }
        $(document).ready(function () {
            ...
            var data = {};
            var url = "<?php echo base_url(); ?>index.php/quyen/data0";
            // prepare the data
            var source =
            {
                datatype: "json",
                ...
                updaterow: function (rowid, rowdata, commit) {
                    ...
                    var data = "update=true&Q_TEN=" + rowdata.Q_TEN;
                    data = data + "&Q_MA=" + rowdata.Q_MA;
                    console.log(data);
                    $.ajax({
                        dataType: 'json',
                        url: '<?php echo base_url(); ?>index.php/quyen/data0',
                        data: data,
            ...
            $("#jqxgrid").jqxGrid(
            {
                ...
                columns: [
                    {
                      text: "<?php echo lang('key') ?>", columntype: 'textbox', datafield: 'Q_MA', width: "30%", cellsalign: 'center', align: "center", editable: false,
                    ...
                    {
                      text: "<?php echo lang('name') ?>", columntype: 'textbox', datafield: 'Q_TEN', width: "56%", cellsalign: 'left', align: "left",
                      validateEverPresentRowWidgetValue: function (datafield, value, rowValues) {
                          if (!value) {
            ...
            $("#popupWindow").on('open', function () {
                $("#Q_MA").jqxInput('selectAll');
            });
         
            $("#Cancel").jqxButton({ });
            $("#Save").jqxButton({ });
            $("#Q_MA").jqxInput({ height: "20" });
            $("#Q_TEN").jqxInput({ height: "20" });
            // update the edited row when the user clicks the 'Save' button.
            $("#Save").click(function () {
                if (editrow >= 0) {
                    var row = { Q_MA: $("#Q_MA").val(), Q_TEN: $("#Q_TEN").val()
                    };
